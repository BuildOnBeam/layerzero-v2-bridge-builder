...
def main():
    ...
    name = get_user_input("Enter the name of the token (e.g. MyToken)","Test")
    symbol = get_user_input("Enter the symbol of the token (e.g. MYT)","TST")
    delegate = get_user_input("Enter the delegate address","0x45f16A36E11B60a15B7E524249Eded7E1a084502")
    percentage = get_user_input("Enter the bridge fee basis points. 100% would be 1Mil, 10% is 100k, 1% is 10k...", "0")
    ...
    chain_id_a = int(get_user_input("Enter Chain ID for the chain to deploy OFT on. It is the \"destination\" chain of the token to bridge",13337))
    sharedDecimals = int(get_user_input("Shared decimals - only needed if the token max supply exceeds 18.45T",6))
